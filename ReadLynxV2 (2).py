...
## Micron resolution 600 by 600 screen and a 30 cm by 30 cm, detector resolution problem
## 1000 pixels by 1000 pixels, 0.625 mm with very many slices of ct data, 
import pydicom
...
while iCounter < iNumFiles:
    ...
    ax1 = plt.subplot(221) # (nrows, ncols, index)
    ax2 = plt.subplot(222)
    ax4 = plt.subplot(224)
    ...
    for peak_slice in peak_slices:
        ...
        vProfilex=mPix[int(cx1-iBuffer):int(cx1+iBuffer+1),int(cy1)]
        vProfiley=mPix[int(cx1),int(cy1-iBuffer):int(cy1+iBuffer+1)]
        cy_mm=(cy+y)*0.5
        cx_mm=(cx+x)*0.5
